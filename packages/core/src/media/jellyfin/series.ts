...
export async function getSeriesWatchHistory(
  ...
  const upsertEpisode = (item: JellyfinEpisode) => {
    const mapped = watchedEpisodeFromJellyfinItem(item)
  ...
  const resumeResponse = await provider.fetch<{ Items: JellyfinEpisode[] }>(
  ...
  for (const item of favoritesResponse.Items) {
    if (!itemsMap.has(item.Id)) {
      const favorite = watchedEpisodeFromJellyfinItem(item)
...
function watchedEpisodeFromJellyfinItem(item: JellyfinEpisode): WatchedEpisode {
  return {
    episodeId: item.Id,
