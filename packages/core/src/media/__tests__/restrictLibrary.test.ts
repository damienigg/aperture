...
import { restrictLibraryToOwner } from '../emby/libraries.js'
...
class MockEmbyProvider extends EmbyProvider {
  async fetch<T>(_endpoint: string, _apiKey: string, _options?: unknown): Promise<T> {
    return {} as T
  }
}
...
test('restrictLibraryToOwner should restrict library access to owner only', async () => {
  ...
  const fetchMock = mock.method(mockProvider, 'fetch')
